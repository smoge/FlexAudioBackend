...
AudioBackend::AudioFormat JackBackend::get_current_audio_format() const {
    ...
    return AudioFormat{static_cast<unsigned int>(jack_get_sample_rate(client_)),
...
float JackBackend::get_cpu_load() const { return client_ ? jack_cpu_load(client_) : 0.0f; }

size_t JackBackend::get_buffer_size() const { return client_ ? jack_get_buffer_size(client_) : 0; }
...
std::string JackBackend::get_error_message() const { return error_message_; }
...
int JackBackend::jack_process_callback(jack_nframes_t nframes, void* arg) {
    ...
    if (backend->process_callback_) {
        const float* buffer =
            static_cast<const float*>(jack_port_get_buffer(backend->input_port_, nframes));
