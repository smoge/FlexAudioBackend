...
void run_pipewire_loop(AudioBackend* backend) {
    backend->activate();
}
...
int main(int argc, char* argv[]) {
    ...
    if (!audio_backend->initialize("SimpleClient", {48000, 1, AudioBackend::AudioFormat::SampleFormat::Float32})) {
        std::cerr << "Failed to initialize audio backend: " << audio_backend->get_error_message() << std::endl;
        return 1;
    }

    audio_backend->set_process_callback([&processor](const float* buffer, size_t nframes) {
        processor.process(buffer, nframes);
    });
    ...
    if (backend_type == "pipewire") {
        ...
        if (!audio_backend->activate()) {
            std::cerr << "Failed to activate audio backend: " << audio_backend->get_error_message() << std::endl;
            return 1;
        }
    }
    ...
    while (running) {
        ...
        std::cout << "\rVolume: [" << std::string(filled_width, '#') << std::string(meter_width - filled_width, ' ')
                  << "] " << std::fixed << std::setprecision(1) << db << " dB" << std::flush;
